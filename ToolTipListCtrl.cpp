...
CToolTipListCtrl::CToolTipListCtrl()
{
	...
	m_pchTip = NULL;
	m_pwchTip = NULL;
	//::SendMessage(this->hwnd, TTM_SETMAXTIPWIDTH, 0, 300);
}
...
BOOL CToolTipListCtrl::OnToolTipText( UINT id, NMHDR * pNMHDR, LRESULT * pResult ){
  ...
  //      #ifndef _UNICODE
  //	      if (pNMHDR->code == TTN_NEEDTEXTA)
  //		      lstrcpyn(pTTTA->szText, strTipText, 512);
  //	      else
  //		      _mbstowcsz(pTTTW->szText, strTipText, 512);
  //      #else
  //	      if (pNMHDR->code == TTN_NEEDTEXTA)
  //		      _wcstombsz(pTTTA->szText, strTipText, 512);
  //	      else
  //		      lstrcpyn(pTTTW->szText, strTipText, 512);
  //      #endif
