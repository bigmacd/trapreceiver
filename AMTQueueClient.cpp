...
void  
AMTQueueClient::Put(unsigned char* message, unsigned int length)
{
  unsigned long len = 0;
...
BOOL
AMTQueueClient::Connect()
{
  BOOL retVal = FALSE;
  
  mQueueHandle = CreateFile("\\\\.\\mailslot\\trap\\guiToAMT",
			    GENERIC_WRITE,
			    FILE_SHARE_READ,
			    NULL,
			    OPEN_EXISTING,
